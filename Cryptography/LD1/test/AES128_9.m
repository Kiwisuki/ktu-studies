...
% >> keyd=12345;
% >> key=dec2hex(keyd,32)
% key = 00000000000000000000000000003039
% >> 3*16^3+3*16+9
% ans = 12345
% >> keyd=1234567
% keyd = 1234567
% >> key=dec2hex(keyd, 32)
% key = 0000000000000000000000000012D687
% >> 1*16^5+2*16^4+13*16^3+6*16^2+8*16+7
% ans = 1234567
%
% Nr - number of rounds (e.g. Nr = 10)
% EnDec - letter determining either encription or decryption
%
% Example:
% >> key = '0000000000000000000000000012D687';
% >> in = 'Laba diena';
...
% 7742110fdf5c568250f2673e7bb28ae0 % ciphertext in hex format
% C = wBß\V?P?g>{²Š?     % ciphertext in ASCII format
% >> D = AES128(C,key,Nr,'d')
% D = Laba diena
%
