% C = AES128(in,key,NR,'mode')
%
% in - string type text/ciphertext maximum 16 symbols or shorter
%
% key - shared secret key in hexadecimal number of length=32 symbols (128 bits)
%       having common secret key K key can be obtained using function:
%       >> key=dec2hex(decimal_number,32)
%          For example:
%       >> K=int64(12345678)
%       K = 12345678
%       >> key=dec2hex(K,32)
%       key = 00000000000000000000000000BC614E
%
% NR - number of rounds (e.g. NR = 10). To speed up a computations NR=1 
%
%  
% 'mode' - 'e' for encryption
%          'd' for decryption
%
...
% new = &4X./mzO            % ciphertext in ASCII format
% C = 26345801ae9d12172e2f6dcd7a0ee34f   % ciphertext in hex format
%
% Example of decryption:
% >> D = AES128(C,key,NR,'d')
% Out = 0000000000004c616261206469656e61   % plaintext in hex format
% D = Laba diena                            % plaintext
%
%
function Out = AES128(in, key ,Nr, mode)

if(length(in) ~= 32)
in = dec2hex(double(in));
inn = '';
index = 0;
for i = 1:16
  if(i <= (16-length(in)))
...
if(lower(mode) == 'e')
Out = Cipher(key,in, Nr);
new = '';
for i = 1:16
new = cstrcat(new, char(hex2dec(Out((i-1)*2+1:(i-1)*2+2))));
end
display(new);
else if(lower(mode) == 'd')
Out = InvCipher(key,in, Nr)
Dm = '';
tikr = 0;
for i = 1:16
if(sum(Out((i-1)*2+1:(i-1)*2+2) ~= '00')~=0 || tikr)
smb = char(hex2dec(Out((i-1)*2+1:(i-1)*2+2)));
Dm = cstrcat(Dm,smb);
tikr = 1;
endif
end
...
for k=2:Nr%(Nk+6)%conducts follow-on rounds
...
state=AddRoundKey(state,w(:,4*Nr+1:4*(Nr+1)));%(Nk+6)+1:4*(Nk+7)));
Out=state(:);%changes output to column vector
Out=lower(dec2hex(Out(1:length(In)),2)');%converts output to hex
Out=Out(:)';%converts output to row vector
end

function Out = InvCipher(key,In, Nr)
...
Out=state(:)';
Out=lower(dec2hex(Out(1:length(In)),2)');
Out=Out(:)';
end
