...
if(length(in) ~= 32)
in = unicode2native(in, 'ISO-8859-1');
inn = '';
index = 0;
for i = 1:16
  if(i <= (16-length(in)))
  ...
  inn = strcat(inn, dec2hex(in(i-index),2));
  end
end
in = lower(inn);
end
...
for i = index:2:32
  rez = strcat(rez, native2unicode(hex2dec(Out(i:i+1)), 'ISO-8859-1'));
